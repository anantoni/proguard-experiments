...
public class ColdAdvanceActivity extends SwipeRightFragmentActivity {
    ...
    private SettingSelectorView.SettingSelector importPrivateKeySelector = new
            SettingSelectorView.SettingSelector() {
                ...
                public void onOptionIndexSelected(int index) {
                    hasAnyAction = true;
                    int i = getStringResouceForIndex(index);
                    if (i == R.string.import_private_key_qr_code) {
                        importPrivateKeyFromQRCode();
                        return;
                    } else if (i == R.string.import_private_key_text) {
                        importPrivateKeyFromText();
                        return;
                    } else if (i == R.string.import_hdm_cold_seed_qr_code) {
                        importHDMColdFromQRCode();
                        return;
                    } else if (i == R.string.import_hdm_cold_seed_phrase) {
                        importHDMColdFromPhrase();
                        return;
                    } else if (i == R.string.import_cold_hd_account_seed_qr_code) {
                        importHDFromQRCode();
                        return;
                    } else if (i == R.string.import_cold_hd_account_seed_phrase) {
                        importHDFromPhrase();
                        return;
                    } else {
                        return;
